...
async function run() {
  console.log("=== SEARCHING DESKTOP FILES FOR ROOM 217 EMAILS ===");
  const emails = [
    'ranaaman9153@gmail.com',
    'abhaykumar1062003@gmail.com',
    'vikashkumarverma579@gmail.com'
  ];
  ...
  for (const file of files) {
    if (file.includes('inspect') || file.includes('smart-match') || file.includes('search-erp')) continue;
    try {
      ...
      for (const sheetName of wb.SheetNames) {
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          for (const email of emails) {
            if (rowStr.includes(email.toLowerCase())) {
